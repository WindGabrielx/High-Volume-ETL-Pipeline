...
for i in range(number_of_department):
    ...
    a = [s for s in sensor_list if s not in check_list] # ตรงนี้ไม่ได้ใช้
    data_department = pd.DataFrame({
        'department_name' : [d_name] * n_sensor
        , 'sensor_serial' : random.choices(
            [s for s in sensor_list if s not in check_list] # ตรงนี้จริงๆต้องเปลี่ยนเป็น (a, k=n_sensor)
            , k = n_sensor
            )
        })
...
for i in tqdm(range(len(list_date))):
    ...
    path = os.path.join(folder_name, str(create_at).replace(':', '-').replace(' ', '_')) + '.parquet'
